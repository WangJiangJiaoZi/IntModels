...
const removeIbDiv = (divGroup, language = 1) => {
	...
	if (indexOfDivToRemove < 0) {
		...
		let message;
		if (language === 1) {
			message = "Cannot remove inbound divider "
					+ "when updating dividerGroup geometry. There is no an inbound divider "
					+ " to remove.";
		}
		else {
			message = "无法移除进口方向隔离带，因为不存在此隔离带。";
		}
