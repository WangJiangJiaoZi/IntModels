...
const addObDiv = (divGroup, type, color, offset, startWidth, capRadius, language = 1) => {
	//check if there is an outbound divider already:
	let ifAlreadyExists = false;
	divGroup.dividers.forEach((oneDiv) => {
		ifAlreadyExists = (oneDiv.dividerType === 2) ? true : false;
		if (ifAlreadyExists) {
			const modelOwner = "DividerGroup";
			let message;
			if (language === 1) {
				message = "Cannot add a new outbound divider "
						+ "when updating dividerGroup geometry. There has been an"
						+ " existing one.";				
			}
			else {
				message = "无法添加新的出口方向隔离带，因为该隔离带已经存在了。";
			}
	...
	const obDivider = new Divider(divGroup, divGroup.THREE);
	const appId = divGroup.approach.appId;
	verifyDivGeo(ibDivGeo, appId);
	obDivider.dividerControllers.injectDivGeo(obDivider, ibDivGeo);

	divGroup.dividers.push(obDivider);
};


export default addObDiv;
